...
init_vimeo_config() {
    ...
    if [[ -f "$config_file" ]] && source "$config_file" && [[ -n "$VIMEO_ACCESS_TOKEN" ]]; then
        # Configuration exists, validate it's still working
        echo "🔍 Validating existing Vimeo configuration..."
        if test_vimeo_connection_silent; then
            echo "✅ Using existing Vimeo configuration"
            return 0
        else
            echo "⚠️  Existing token appears to be invalid, requesting new one..."
        fi
    fi
    ...
    echo "================================"
    echo ""
    echo "To use the Vimeo uploader, you need a personal access token:"
    echo ""
    echo "📋 Setup Steps:"
    ...
    while true; do
        read -p "Enter your Vimeo Personal Access Token: " vimeo_token
        ...
        if test_vimeo_connection_silent; then
            ...
            cat > "$config_file" << EOF
# Vimeo Configuration for Vyn v1.4.0
# This file is automatically generated
# Personal Access Token for Vimeo API
VIMEO_ACCESS_TOKEN="$vimeo_token"
EOF
            
            echo "✅ Token validated and saved to $config_file"
            echo "🎉 Vimeo uploader is now ready to use!"
            echo ""
            return 0
        else
            echo "❌ Token validation failed"
            echo "Please check that your token has the required scopes:"
            echo "   - Upload videos"
            echo "   - Create/edit albums"
            echo "   - Private access to your account"
            echo ""
            read -p "Try again with a different token? (y/N): " retry
            if [[ ! "$retry" =~ ^[Yy]$ ]]; then
                echo "Setup cancelled"
                return 1
            fi
...
# Test Vimeo API connection (silent version for validation)
test_vimeo_connection_silent() {
    local response=$(curl -s -H "Authorization: bearer $VIMEO_ACCESS_TOKEN" \
                          -H "Accept: application/vnd.vimeo.*+json;version=3.4" \
                          "$VIMEO_API_BASE/me" 2>/dev/null)
